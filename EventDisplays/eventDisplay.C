...
void eventDisplay() {
  ...
  for( auto& dobj : dobjs ) { //loop over trees
    ...
    while( entry < dobj->tree->GetEntriesFast() ) {
      ...
      for( size_t iv = 0; iv < dobj->vtx_ntrk->size(); iv++ ) { // loop over vertices 
        ...
        //std::cout << "size tracks " << trk_pt_wrtSV->size() << std::endl;
        int matched_ntrks = 0;
        for( size_t itrk=0; itrk < trk_pt_wrtSV->size(); itrk++) {
          ...
          //std::cout << __FUNCTION__ << " trk index " << itrk << std::endl;

          // check if track is from a muon
          bool isTrackAMuon = false;
          for( size_t imu = 0; imu < muon_d0->size(); imu++ ) { 

            if ( isMuon( trk_d0->at(itrk), trk_z0->at(itrk), muon_d0->at(imu), muon_z0->at(imu) ) ) {
              isTrackAMuon = true;
              //print_event==true;
            }
          }// end loop on muons
      ...
      /*
      // LOOP OVER MUONs
      for( size_t imu = 0; imu < dobj->muon_d0->size(); imu++ ) { // loop over muons 
  
        std::cout << __FUNCTION__ << " muon index " << imu << std::endl;
          
        const auto& d0  = dobj->muon_d0 ->at(imu);
        const auto& z0  = dobj->muon_z0 ->at(imu);
        const auto& pt  = dobj->muon_pt ->at(imu);
        const auto& eta = dobj->muon_eta->at(imu);
        const auto& phi = dobj->muon_phi->at(imu);
        const auto& q   = dobj->muon_charge->at(imu);
        
        // Trying this for muon line
        std::cout << "d0 " << d0 << std::endl;
        std::cout << "x " << d0*cos(phi) << std::endl;
        std::cout << "y " << d0*sin(phi) << std::endl;
        std::cout << "phi " << phi << std::endl;

        // somethings weird
        auto* line = new TGraph;
        TVector3 p; p.SetPtEtaPhi( pt, eta, phi );
        line->SetPoint(0, d0*cos(phi), d0*sin(phi) );
        line->SetPoint(1, d0*cos(phi) + p.Perp()*cos(p.Phi())*20, d0*sin(phi) + p.Perp()*sin(p.Phi())*20 );
        muon_trks.at(panel).emplace_back( line );
